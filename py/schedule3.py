...
dirs = filter(os.path.isdir, os.listdir ( url_path )) # Saves a list of the directories in the defined directory in a variable
...
def get_latest_release():
    ...
    for i in dirs: # Creating a for loop to go through each file in the path
        a = os.stat(os.path.join(url_path,i)) #os.stat returns file attributes about an inode. os.path.join joins one or more path components
        file_list.append([time.ctime(a.st_ctime)]) #time of most recent metadata change on Unix, or the time of creation on Windows)

new_path = (os.path.join(url_path,i)) # Defining the new path		
print (new_path)
